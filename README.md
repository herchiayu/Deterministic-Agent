...
### 白名單與權限控制（`backend/config/access_control.csv`）

```csv
Username,ReadGroups,WriteGroups
Boss,All,Public
Herch,Developer;RD_ME,Developer;RD_ME
User_All,Developer;RD_ME,Developer;RD_ME
User_RD,RD_ME,RD_ME
User_Dev,Developer,Developer
User_None,Public,
```

- **ReadGroups**：RAG 聊天時能查詢到的知識庫群組。
- **WriteGroups**：在 Knowledge-Base-Manager 子站能管理（新增、編輯、刪除）的群組。
- `All`：ReadGroups 填入 `All` 代表可讀取所有群組（動態探索 `data/` 下的子目錄）。
- `Public`：所有名單上的使用者自動獲得 `Public` 的讀取權限，不需手動填寫。
- ReadGroups 與 WriteGroups 皆可留空：留空時僅有 Public 讀取權／無寫入權。
- 若 `access_control.csv` 不存在，則不啟用白名單（所有登入者皆可進入）。

#### 權限模型範例

| 帳號 | ReadGroups | WriteGroups | RAG 可查 | KBM 可管理 |
|------|-----------|------------|---------|----------|
| Boss | All | Public | 所有群組 | Public |
| Herch | Developer;RD_ME | Developer;RD_ME | Developer、RD_ME、Public | Developer、RD_ME |
| User_None | Public | （空） | Public | （無） |
...
├── Public/                  ← 公共知識庫（所有名單上的使用者皆可讀取）
│   ├── system/              ← 系統管理員手動放置
│   │   └── notice.md
│   └── {username}/          ← 有 Public 寫入權的使用者
│       └── *.md / *.txt
├── {group}/
...
| 情境 | RAG 聊天 | KBM 管理 |
|------|---------|----------|
| ReadGroups 含 `RD_ME` | 可查到 `RD_ME/` 下所有成員的知識 | — |
| ReadGroups 含多個群組 | 合併所有群組的知識庫檢索 | — |
| ReadGroups = `All` | 查詢所有群組 | — |
| WriteGroups 含 `Developer` | — | 可管理 `Developer/` 下自己的檔案 |
| WriteGroups 為空 | — | 無法進入 KBM（顯示「請聯絡管理員」） |
| `Public` 群組 | 所有名單上的使用者自動可讀 | 僅有 WriteGroups 含 `Public` 的使用者可管理 |
| 知識衝突 | AI 分別列出各方說法與來源，不自行裁定 | — |
...
每個群組對應一個 collection，名稱即群組名稱（如 `Developer`、`RD_ME`、`Public`）。Chunk ID 格式為 `{uploader}/{filename}::chunk_{n}`。
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| `GET` | `/api/check-auth` | 確認登入狀態（含 write_groups） |
| `POST` | `/api/logout` | 登出 |
| `GET` | `/api/files?group={group}` | 列出使用者在指定群組的檔案（需 write 權限） |
| `GET` | `/api/file/content?group={group}&name={file}` | 讀取檔案內容（需 write 權限） |
...
| `PUBLIC_GROUP` | `'Public'`（所有名單上的使用者自動可讀取） |
| `ALL_GROUPS_KEYWORD` | `'All'`（CSV 中代表讀取所有群組） |
...
1. 在 `backend/config/access_control.csv` 新增使用者並在 ReadGroups / WriteGroups 設定群組名稱。
...
- **`Public` 群組自動訂閱**：所有名單上的使用者自動取得 `Public` 讀取權，但僅有 WriteGroups 含 `Public` 的使用者才能在 KBM 管理其內容。
- **讀寫權限分離**：ReadGroups 控制 RAG 可查詢範圍，WriteGroups 控制 KBM 可管理範圍，兩者獨立設定。
